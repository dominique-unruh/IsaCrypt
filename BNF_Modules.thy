...
proof (rule predicate2I)
  ...
  proof (induction rule:beta_reduce_proc.induct)
    case (br_Proc i s t)
    hence "proc_relation_nth (\<Rightarrow>>) i s t"
      apply (rule predicate2D[where Q="proc_relation_nth (\<Rightarrow>>) i", rotated]) 
    ...
    ultimately have "list_all2 (\<Rightarrow>>) (proc_list s) (proc_list t)" 
      apply (rule_tac list_all2_all_nthI)
       apply (metis upd length_list_update)
      by (case_tac "n=i", auto)
    with shape have "proc_relation (\<Rightarrow>>) s t" unfolding proc_relation_def by auto
...
proof (induct p arbitrary: p' n)
  ...
  proof (erule_tac par_beta_cases)
    fix s' assume p': "p' = Proc s'" and p1_s': "proc_relation (\<Rightarrow>>) p1 s'"
    ...
    hence "proc_relation (\<Rightarrow>>) (proc_map (\<lambda>p. lift_proc p n) p1) (proc_map (\<lambda>p. lift_proc p n) s')"
...
proof (induct t arbitrary: s s' t' n)
  case (Proc p) 
  from Proc.prems(2) 
  obtain u' where t': "t' = Proc u'" and pu': "proc_relation (\<Rightarrow>>) p u'"
  ...
  hence "proc_relation (\<Rightarrow>>) (proc_map (subst_proc n s) p) (proc_map (subst_proc n s') u')"
...
proof -
  ...
  next case (pb_Proc s s') hence Procsz: "Proc s \<Rightarrow>> z" by auto
    ...
    hence "list_all2 (\<Rightarrow>>) (proc_list s') (proc_list u')"
    ...
    hence "list_all2 (\<Rightarrow>>) (proc_list z') (proc_list u')"
