...
proof (unfold rhoare_def, rule, rule, rule)
  ...
  def witness == "product_distr (denotation c m1) (denotation Skip m2)"
  show " \<exists>\<mu>. apply_to_distr fst \<mu> = denotation c m1 \<and>
             apply_to_distr snd \<mu> = denotation Skip m2 \<and>
    ...
    apply (metis `lossless c` lossless_def scaleR_one_distr)
...
proof (unfold rhoare_def, rule, rule, rule)
  ...
  obtain witness where wit2: "apply_to_distr fst witness = denotation c2 m2"
                   and wit1: "apply_to_distr snd witness = denotation c1 m1"
  ...
  have wit'1: "apply_to_distr fst witness' = denotation c1 m1"
    unfolding witness'_def wit1[symmetric] apply auto
    apply (rule cong_middle[where f=apply_to_distr])
    by auto
  have wit'2: "apply_to_distr snd witness' = denotation c2 m2"
  ...
  show "\<exists>\<mu>. apply_to_distr fst \<mu> = denotation c1 m1 \<and>
                  apply_to_distr snd \<mu> = denotation c2 m2 \<and> (\<forall>m1' m2'. (m1', m2') \<in> support_distr \<mu> \<longrightarrow> Q m1' m2')"
...
proof -
  ...
  show "apply_to_distr fst \<mu>' = denotation (Sample x d) m1"
    unfolding \<mu>'_def apply simp
    unfolding mu1[symmetric] apply simp
    apply (rule cong_middle[where f=apply_to_distr])
    unfolding map_def by auto
  show "apply_to_distr snd \<mu>' = denotation (Sample y e) m2" 
    unfolding \<mu>'_def apply simp
...
proof (unfold rhoare_def, auto (*, rule exI, auto*))
  ...
  obtain \<mu>1 where \<mu>1fst: "apply_to_distr fst \<mu>1 = denotation c1 m1"
              and \<mu>1snd: "apply_to_distr snd \<mu>1 = denotation c m"
              and \<mu>1supp: "\<And>m1' m'. (m1',m') \<in> support_distr \<mu>1 \<Longrightarrow> Q1 m1' m'";
    using `P1 m1 m` rhl1 unfolding rhoare_def by metis
  obtain \<mu>2 where \<mu>2fst: "apply_to_distr fst \<mu>2 = denotation c m"
              and \<mu>2snd: "apply_to_distr snd \<mu>2 = denotation c2 m2"
...
lemma rcase_rule:
  assumes "\<And>x. rhoare (\<lambda>m1 m2. P m1 m2 \<and> f m1 m2 = x) c1 c2 Q"
  shows "rhoare P c1 c2 Q"
using assms unfolding rhoare_def by metis
...
lemma rseq_rule1:
  assumes "\<And>m1 m2. Q m1 m2 \<Longrightarrow> Q' m1 m2"
      and "rhoare P c1 Skip Q"
      and "rhoare Q' Skip c2 R"
  shows "rhoare P c1 c2 R"
sorry


lemma rif_rule:
  assumes "\<And>m1 m2. P m1 m2 \<Longrightarrow> eu_fun e1 m1 = eu_fun e2 m2"
      and "rhoare (\<lambda>m1 m2. P m1 m2 \<and> eu_fun e1 m1 = embedding True) then1 then2 Q"
      and "rhoare (\<lambda>m1 m2. P m1 m2 \<and> eu_fun e1 m1 \<noteq> embedding True) else1 else2 Q"
